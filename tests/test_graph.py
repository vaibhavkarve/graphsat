...
st.register_type_strategy(Vertex, st.integers(min_value=1).map(vertex))
...
def test_edge_on_integer_collection_input(instance: Collection[int]) -> None:
    ...
    assert edge(instance) == set(map(vertex, instance))
...
st.register_type_strategy(
    Graph,
    st.frozensets(st.from_type(Edge), min_size=1).map(graph))
